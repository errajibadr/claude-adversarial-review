...
settings before submitting the prompt. Enforcement comes from Claude's native
sandbox, the operating system, and the CLI honoring `failIfUnavailable`.
...
runner's canonical read-only Git commands through Claude's native Bash sandbox.
...
That sandbox denies writes to the reviewed repository and resolved Git metadata
and blocks subprocess network
access. Write/edit tools are unavailable and mutation commands are not authorized;
private runtime working/configuration paths may still be written for bookkeeping.
Tests, arbitrary commands, and sandbox fallbacks are not authorized by the review
workflow. Snapshot mode grants only the three file tools;
...
If an enclosing Codex sandbox hides a known working login, use normal
approval-controlled execution for the same restricted invocation when allowed.
Never copy credentials, bypass permissions, or disable TLS verification. If the
CLI or host cannot enforce the required controls, report the review as blocked.
